...
class cl_textureWidth : public R_constant_setup
{
    ...
    {
        const float tW = (float)FluidManager.GetTextureWidth();
        cmd_list.set_c(C, tW);
    }
};
...
class cl_textureHeight : public R_constant_setup
{
    virtual void setup(CBackend& cmd_list, R_constant* C)
    {
        const float tH = (float)FluidManager.GetTextureHeight();
        cmd_list.set_c(C, tH);
    }
};
...
class cl_textureDepth : public R_constant_setup
{
    virtual void setup(CBackend& cmd_list, R_constant* C)
    {
        const float tD = (float)FluidManager.GetTextureDepth();
        cmd_list.set_c(C, tD);
    }
};
...
class cl_maxDim : public R_constant_setup
{
    virtual void setup(CBackend& cmd_list, R_constant* C)
    {
        const int tW = FluidManager.GetTextureWidth();
        const int tH = FluidManager.GetTextureHeight();
        const int tD = FluidManager.GetTextureDepth();
        const float tMax = (float)_max(tW, _max(tH, tD));
...
{
    LPCSTR* TNames = dx103DFluidManager::GetEngineTextureNames();
    LPCSTR* RNames = dx103DFluidManager::GetShaderTextureNames();
