...
void CRenderTarget::phase_ssfx_bloom(CBackend& cmd_list)
{
    ...
    if (mask_control)
    {
        ...
        {
            RenderScreenTriangle(cmd_list, *rt_LensBlur[lensblur], s_ssfx_bloom_lens->E[1 + lensblur], [&]() { cmd_list.set_c("blur_setup", w / 4, h / 4, 0, 2.0f + (3.0f * lensblur)); });
        }
    }
    ...
    for (int downsample = 0; downsample < 6; downsample++)
    {
        ...
        RenderScreenTriangle(cmd_list, *rt_Down[downsample], s_ssfx_bloom_downsample->E[downsample], [&]() { cmd_list.set_c("blur_setup", w / SampleScale, h / SampleScale, 0, ps_ssfx_bloom_2.x); });
    }
    ...
    for (int upsample = 0; upsample < 5; upsample++)
    {
        ...
        set_viewport_size(cmd_list, w / SampleScale, h / SampleScale);
        RenderScreenTriangle(cmd_list, *rt_Up[upsample], s_ssfx_bloom_upsample->E[upsample], [&]() { cmd_list.set_c("blur_setup", w / SampleScale, h / SampleScale, 0, ps_ssfx_bloom_2.x); });
    }

    // BLOOM COMBINE ///////////////////////////////////////////////
    RenderScreenTriangle(cmd_list, rt_ssfx_bloom1, s_ssfx_bloom->E[1], [&]() { cmd_list.set_c("mask_control", static_cast<float>(mask_control), static_cast<float>(visor_control), ps_r2_visor_refl_intensity, ps_r2_visor_refl_radius); });
